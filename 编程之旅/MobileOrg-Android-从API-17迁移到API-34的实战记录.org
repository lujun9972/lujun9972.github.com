...
本次迁移全程由 [[https://claude.ai/code][Claude Code]] 辅助完成。从构建系统升级到运行时兼容性修复，Claude Code 在以下方面提供了巨大帮助：

- 快速定位 Android 版本兼容性问题
- 系统性地搜索同类问题（如全局搜索所有 =NotificationCompat.Builder= 调用）
- 在修复一个 bug 时预判可能引入的新问题（如 early return 导致的 NPE）
- 将每个修复经验记录到 CLAUDE.md 中，避免重复踩坑

当然，Claude Code 也犯了错误——比如只修了一个菜单文件的 =showAsAction= 就以为搞定了。这时候需要人工介入，做全局检查。AI 是好助手，但不能替代人类的判断力。
