...
* artbollocks-mode
artbollocks-mode是一款检查写作风格的第三方插件。它能高亮出那些你认为不好的，需要注意的语言模式。
比如，它能检测到重复出现的单词(artbollocks-lexical-illusions-regex)，而这可能是你一不小心多写了。
...
不过我们一般写作过程中，也不太会用到艺术评论方面的行话，因此我们可以手工将之清空，或者使用自己领域的行话进行代替
#+BEGIN_SRC emacs-lisp
  (setq artbollocks-jargon nil)           ;关闭检测行话功能
  ;; 或者
  (setq artbollocks-jargon-regex nil)     ;设定自己的行话列表
#+END_SRC
