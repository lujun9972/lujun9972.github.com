...
这里要解决的是上面提到的第三个问题: *多进程写入文件时如何保持不乱序*
为此，我们需要在写入文件时对文件加锁，然后在写完文件后对文件解锁，在shell中我们可以使用 =flock= 来对文件进行枷锁。
使用 =flock= 最简单的方式就是 =flock 文件或目录 命令 [参数...]=, 它会执行 =命令 参数...= 并在命令执行期间对 =文件或路径= 加排他锁，然后在命令执行完毕后再释放该锁
...
*这里需要注意的是，使用flock对文件加锁的情况下其他程序依然可以在不用flock的情况下写如该文件*,比如
