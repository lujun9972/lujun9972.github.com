...
2. 使用 =wpa_passphrase= 生成配置
   ...
   #+BEGIN_SRC shell  :dir /var/media/lujun9972/linux
   ...
   可以配置多个wifi
   #+BEGIN_SRC shell  :dir /var/media/lujun9972/linux
     sudo wpa_passphrase MERCURY_6220_201 $WIFIPWD |sudo tee -a etc/network/wpa_passphrase.conf
   #+END_SRC
...
3. 将wlan加入 =etc/network/interfaces= 中
   ...
   至此wifi就设置好了,推出TF卡，插入Orange Pi Lite后通电，Orange Pi Lite就会自动连接上wifi了
